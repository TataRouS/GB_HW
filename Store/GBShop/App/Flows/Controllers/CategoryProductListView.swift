...
//  GBShop
//
//  Created by Nata Kuznetsova on 14.07.2023.
//
        ...
        catalog.getCatalog(pageNumber: 1, categoryId: 1, category:category) { response in
            ...
            case .failure(let error):
                let keysAndValues = [
                              ...
                              Crashlytics.crashlytics().log("get_catalog_in_crash")
        ...
        if let indexPath = productTableView.indexPath(for: cell) {
            ...
            addProductToBasket.addProductToBasket(productId: 123, quantity: 1) { response in
                ...
                case .success(let result):
                                    ...
                                    Analytics.logEvent("add_product_to_basket_success",
                ...
                case .failure(let error):
                    let product = self.products[indexPath.row]
                                      ...
                                      Crashlytics.crashlytics().setCustomKeysAndValues(keysAndValues)
                                      Crashlytics.crashlytics().log("add_product_to_basket_in_crash")
                                      fatalError(error.localizedDescription)
                }
            }
        }
